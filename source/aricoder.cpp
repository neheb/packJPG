...
template <std::uint8_t bit> void ArithmeticBitWriter::write_bit()
{
...
{
	constexpr std::uint8_t all_ones =
...
void ArithmeticEncoder::encode(symbol *s)
{
	...
	while (clow_local >= CODER_LIMIT050 || chigh_local < CODER_LIMIT050) {
		if (chigh_local <
		    CODER_LIMIT050) { // this means both, high and low are below, and 0 can be safely shifted out
			// write 0 bit
			bitwriter_->write_bit<0>();
	...
	while ((clow_local >= CODER_LIMIT025) &&
	       (chigh_local < CODER_LIMIT075)) {
		nrbits++;
...
void ArithmeticDecoder::decode(symbol *s)
{
	...
	// if both, low and high are either in the lower half or in the higher half
	// one bit can be safely shifted out
	while ((clow_local >= CODER_LIMIT050) ||
	       (chigh_local < CODER_LIMIT050)) {
...
model_s::model_s(int max_s, int max_c, int max_o, int c_lim)
	: // Copy settings into the model:
	  max_symbol(max_s), max_context(max_c), max_order(max_o + 1),
	  max_count(c_lim),

	  current_order(max_o + 1), sb0_count(max_s),

	  totals(max_s + 2), scoreboard(new bool[max_s]), contexts(max_o + 3)
{
	std::fill(scoreboard, scoreboard + max_symbol, false);

	// set up null table
	auto null_table = new table_s;
	null_table->counts = std::vector<uint16_t>(
		max_symbol, uint16_t(1)); // Set all probabilities to 1.
	...
	for (int i = 2; i <= max_order; i++) {
		...
		if (i < max_order) {
			contexts[i]->links =
				std::vector<table_s *>(max_context);
		}
...
void model_s::update_model(int symbol)
{
	...
	if (symbol >= 0) {
		for (int local_order = (current_order < 1) ? 1 : current_order;
			...
			context->max_count =
				std::max(count, context->max_count);
			context->max_symbol = std::max(uint16_t(symbol + 1),
						       context->max_symbol);
...
int model_s::convert_symbol_to_int(uint32_t count, symbol *s)
{
	...
	s->low_count =
		totals[c]; // It is guaranteed that there exists such a symbol.
	s->high_count = totals
		[c -
		 1]; // This is guaranteed to not go out of bounds since the search started at index 1 of totals.
	...
	// return symbol value
	return c -
	       2; // Since c is not one and is a positive number, this will be nonnegative.
}
...
model_b::model_b(int max_c, int max_o, int c_lim)
	: // Copy settings into the model:
	  max_context(max_c), max_order(max_o + 1), max_count(c_lim),
