...
#if defined                                                                    \
	BUILD_DLL // define BUILD_LIB from the compiler options if you want to compile a DLL!
#define BUILD_LIB
#endif

#if defined                                                                    \
	BUILD_LIB // define BUILD_LIB as compiler option if you want to compile a library!
...
#define MAX_V(cm, bp)                                                          \
	((QUANT(cm, bp) > 0) ?                                                 \
		 ((freqmax[bp] + QUANT(cm, bp) - 1) / QUANT(cm, bp)) :         \
		 0)
...
INTERN int jpg_encode_block_seq(BitWriter *huffw, huffCodes *dctbl,
				huffCodes *actbl, const short *block);

INTERN int jpg_decode_dc_prg_fs(BitReader *huffr, huffTree *dctree,
				short *block);
INTERN int jpg_encode_dc_prg_fs(BitWriter *huffw, huffCodes *dctbl,
				const short *block);
INTERN int jpg_decode_ac_prg_fs(BitReader *huffr, huffTree *actree,
				short *block, int *eobrun, int from, int to);
INTERN int jpg_encode_ac_prg_fs(BitWriter *huffw, huffCodes *actbl,
				const short *block, int *eobrun, int from,
				int to);

INTERN int jpg_decode_dc_prg_sa(BitReader *huffr, short *block);
INTERN int jpg_encode_dc_prg_sa(BitWriter *huffw, short *block);
INTERN int jpg_decode_ac_prg_sa(BitReader *huffr, huffTree *actree,
				short *block, int *eobrun, int from, int to);
INTERN int jpg_encode_ac_prg_sa(BitWriter *huffw,
				std::vector<std::uint8_t> &storw,
				huffCodes *actbl, const short *block,
				int *eobrun, int from, int to);

INTERN int jpg_decode_eobrun_sa(BitReader *huffr, short *block, int *eobrun,
				int from, int to);
INTERN int jpg_encode_eobrun(BitWriter *huffw, huffCodes *actbl, int *eobrun);
INTERN int jpg_encode_crbits(BitWriter *huffw,
			     std::vector<std::uint8_t> &storw);

INTERN int jpg_next_huffcode(BitReader *huffw, huffTree *ctree);
INTERN int jpg_next_mcupos(int *mcu, int *cmp, int *csc, int *sub, int *dpos,
			   int *rstw);
...
INTERN void jpg_build_huffcodes(const unsigned char *clen,
				const unsigned char *cval, huffCodes *hc,
				huffTree *ht);
...
INTERN bool pjg_encode_generic(ArithmeticEncoder *enc, unsigned char *data,
			       int len);
...
INTERN bool pjg_decode_generic(ArithmeticDecoder *dec, unsigned char **data,
			       int *len);
...
INTERN int pjg_aavrg_context(unsigned short **abs_coeffs, const int *weights,
			     int pos, int p_y, int p_x, int r_x);
INTERN int pjg_lakh_context(signed short **coeffs_x, signed short **coeffs_a,
			    const int *pred_cf, int pos);
...
#if !defined(BUILD_LIB) && defined(DEV_BUILD)
INTERN int collmode =
	0; // write mode for collections: 0 -> std, 1 -> dhf, 2 -> squ, 3 -> unc
INTERN bool dump_hdr(void);
INTERN bool dump_huf(void);
INTERN bool dump_coll(void);
INTERN bool dump_zdst(void);
INTERN bool dump_file(const char *base, const char *ext, void *data, int bpv,
		      int size);
...
INTERN unsigned char *rst_err =
	nullptr; // number of wrong-set RST markers per scan

INTERN unsigned char *zdstdata[4] = {
	nullptr
}; // zero distribution (# of non-zeroes) lists (for higher 7x7 block)
INTERN unsigned char *eobxhigh[4] = {
	nullptr
}; // eob in x direction (for higher 7x7 block)
INTERN unsigned char *eobyhigh[4] = {
	nullptr
}; // eob in y direction (for higher 7x7 block)
INTERN unsigned char *zdstxlow[4] = { nullptr }; // # of non zeroes for first row
INTERN unsigned char *zdstylow[4] = {
	nullptr
}; // # of non zeroes for first collumn
INTERN signed short *colldata[4][64] = {
	{ nullptr }
}; // collection sorted DCT coefficients

INTERN unsigned char *freqscan[4] = {
	nullptr
}; // optimized order for frequency scans (only pointers to scans)
INTERN unsigned char zsrtscan[4][64]; // zero optimized frequency scan

INTERN int adpt_idct_8x8[4][8 * 8 * 8 *
			    8]; // precalculated/adapted values for idct (8x8)
INTERN int adpt_idct_1x8[4][1 * 1 * 8 *
			    8]; // precalculated/adapted values for idct (1x8)
INTERN int adpt_idct_8x1[4][8 * 8 * 1 *
			    1]; // precalculated/adapted values for idct (8x1)
...
INTERN int verify_lv =
	0; // verification level ( none (0), simple (1), detailed output (2) )
INTERN int err_tol =
	1; // error threshold ( proceed on warnings yes (2) / no (1) )
...
INTERN unsigned char nois_trs[4] = { 6, 6, 6,
				     6 }; // bit pattern noise threshold
...
INTERN const char *copyright =
	"2006-2016 HTW Aalen University & Matthias Stirner";
...
int main(int argc, char **argv)
{
	...
	fprintf(msgout, "\n--> %s v%i.%i%s (%s) by %s <--\n", apptitle,
		appversion / 10, appversion % 10, subversion, versiondate,
		author);
	...
	if ((file_cnt == 0) ||
	    ((!developer) &&
	     ((action != A_COMPRESS) || (!auto_set) || (verify_lv > 1)))) {
		show_help();
		return -1;
	}
	...
	if (!auto_set) {
		...
		fprintf(msgout,
			" -------------------------------------------------\n");
		fprintf(msgout,
			" no of segments    ->  %3i[0] %3i[1] %3i[2] %3i[3]\n",
			segm_cnt[0], segm_cnt[1], segm_cnt[2], segm_cnt[3]);
		fprintf(msgout,
			" noise threshold   ->  %3i[0] %3i[1] %3i[2] %3i[3]\n",
	...
	if ((file_cnt > error_cnt) && (verbosity != 0) &&
		...
		cr = (acc_jpgsize > 0) ? (100.0 * acc_pjgsize / acc_jpgsize) :
					 0;
		...
		if (acc_jpgsize > 0) {
			...
			for (int i = 0; i < 4; i++) {
				...
				fprintf(msgout,
					" ac coeffs h [%i] %% : %8.2f %%\n", i,
					100.0 * dev_size_ach[i] / acc_jpgsize);
				fprintf(msgout,
					" ac coeffs l [%i] %% : %8.2f %%\n", i,
					100.0 * dev_size_acl[i] / acc_jpgsize);
				fprintf(msgout,
					" dc coeffs   [%i] %% : %8.2f %%\n", i,
					100.0 * dev_size_dc[i] / acc_jpgsize);
				fprintf(msgout,
					" zero dist h [%i] %% : %8.2f %%\n", i,
					100.0 * dev_size_zdh[i] / acc_jpgsize);
				fprintf(msgout,
					" zero dist l [%i] %% : %8.2f %%\n", i,
					100.0 * dev_size_zdl[i] / acc_jpgsize);
				fprintf(msgout,
					" zero sort   [%i] %% : %8.2f %%\n", i,
					100.0 * dev_size_zsr[i] / acc_jpgsize);
				fprintf(msgout,
					" --------------------------------- \n");
			}
		}
...
#if defined(BUILD_LIB)
EXPORT bool pjglib_convert_stream2mem(unsigned char **out_file,
...
#if defined(BUILD_LIB)
EXPORT void pjglib_init_streams(void *in_src, int in_type, int in_size,
				void *out_dest, int out_type)
{
	...
	case 0:
		...
		break;
	case 1:
		str_in = std::make_unique<MemoryReader>((unsigned char *)in_src,
							in_size);
		break;
	case 2:
		try {
	...
	case 0:
		try {
			str_out =
				std::make_unique<FileWriter>((char *)out_dest);
		} catch (const std::runtime_error &) {
			sprintf(errormessage, "error opening output file %s",
				(char *)out_dest);
			errorlevel = 2;
			return;
		}
	...
	if ((buffer[0] == 0xFF) && (buffer[1] == 0xD8)) {
		...
		filetype = F_JPG;
		// copy filenames
		jpgfilename = (char *)calloc(
			(in_type == 0) ? strlen((char *)in_src) + 1 : 32,
			sizeof(char));
		pjgfilename = (char *)calloc(
			(out_type == 0) ? strlen((char *)out_dest) + 1 : 32,
			sizeof(char));
		strcpy(jpgfilename,
		       (in_type == 0) ? (char *)in_src : "JPG in memory");
		strcpy(pjgfilename,
		       (out_type == 0) ? (char *)out_dest : "PJG in memory");
		...
		jpgfilename = (char *)calloc(
			...
			sizeof(char));
		strcpy(pjgfilename,
		       (in_type == 0) ? (char *)in_src : "PJG in memory");
		strcpy(jpgfilename,
		       (out_type == 0) ? (char *)out_dest : "JPG in memory");
	} else {
		// file is neither
		filetype = F_UNK;
...
EXPORT const char *pjglib_version_info(void)
{
	...
	sprintf(v_info, "--> %s library v%i.%i%s (%s) by %s <--", apptitle,
		appversion / 10, appversion % 10, subversion, versiondate,
		author);
...
EXPORT const char *pjglib_short_name(void)
{
	...
	// copy version info to string
	sprintf(v_name, "%s v%i.%i%s", apptitle, appversion / 10,
		appversion % 10, subversion);
...
INTERN void process_ui(void)
{
	...
	if (verbosity >= 0) { // standard UI
		...
		if (filetype == F_UNK)
			...
			case A_COLL_DUMP:
				...
				break;
			case A_FCOLL_DUMP:
				actionmsg = "Extracting FColls";
				break;
			case A_ZDST_DUMP:
				actionmsg = "Extracting ZDST lists";
				break;
			case A_TXT_INFO:
				actionmsg = "Extracting info";
				break;
			case A_DIST_INFO:
				actionmsg = "Extracting distributions";
				break;
			case A_PGM_DUMP:
	...
	total = static_cast<int>(static_cast<double>((end - begin) * 1000) /
				 CLOCKS_PER_SEC);
	...
	if (verbosity >= 0) { // standard UI
		...
		case 0:
			...
			break;
		case 1:
			errtypemsg = (err_tol > 1) ? "warning (ignored)" :
		...
		if ((verbosity > 0) && (errorlevel < err_tol) &&
		    (action == A_COMPRESS)) {
			if (total >= 0) {
		...
		if (file_no + 1 == file_cnt) {
			// update progress message
			fprintf(msgout, "Processed %2i of %2i files ",
				file_no + 1, file_cnt);
			progress_bar(1, 1);
			fprintf(msgout, "\r");
		}
...
INTERN void show_help(void)
{
	...
	if (developer) {
		...
		fprintf(msgout,
			" [-fcol?] write predicted collections (see above)\n");
...
INTERN bool check_file(void)
{
	...
	try {
		if (pipe_on) {
			...
			str_in = std::make_unique<FileReader>(
				std::string(filename));
		}
	} catch (const std::runtime_error &e) {
	...
	if ((fileid[0] == 0xFF) && (fileid[1] == 0xD8)) {
		...
		// create filenames
		if (!pipe_on) {
			jpgfilename = static_cast<char *>(
				calloc(strlen(filename) + 1, sizeof(char)));
			strcpy(jpgfilename, filename);
			pjgfilename =
				(overwrite) ?
					create_filename(
						filename,
						const_cast<char *>(pjg_ext)) :
					unique_filename(
						filename,
						const_cast<char *>(pjg_ext));
		} else {
		...
		try {
			if (pipe_on) {
				...
				str_out = std::make_unique<FileWriter>(
					std::string(pjgfilename));
			}
		} catch (const std::runtime_error &e) {
			std::strcpy(errormessage, e.what());
			errorlevel = 2;
		...
		// file is neither
		filetype = F_UNK;
		sprintf(errormessage, "filetype of file \"%s\" is unknown",
			filename);
		errorlevel = 2;
		return false;
	}
...
INTERN bool compare_output(void)
{
	...
	const auto result =
		std::mismatch(std::begin(orig_data), std::end(orig_data),
			      std::begin(verif_data), std::end(verif_data));
	if (result.first != std::end(orig_data) ||
	    result.second != std::end(verif_data)) {
		const auto first_diff =
			std::distance(std::begin(orig_data), result.first);
	...
	while (true) {
		if (type == 0xDA) { // if last marker was sos
				...
				if (tmp == 0xFF) {
					...
					if (tmp == 0x00) {
						...
						if (rst_err != nullptr) {
							...
							if (crst > 255) {
								sprintf(errormessage,
									"Severe false use of RST markers (%i)",
									static_cast<
										int>(
										crst));
								errorlevel = 1;
			...
			if (segment[0] != 0xFF) {
				...
				sprintf(errormessage,
					"size mismatch in marker segment FF %2X",
					type);
				errorlevel = 2;
				if (type ==
		...
		if (ssize < len) {
			segment = static_cast<unsigned char *>(
				frealloc(segment, len));
			if (segment == nullptr) {
				sprintf(errormessage, MEM_ERRMSG);
				errorlevel = 2;
				delete (hdrw);
...
INTERN bool decode_jpeg(void)
{
	...
	while (true) {
		...
		for (csc = 0; csc < cs_cmpc; csc++) {
			...
			if (((cs_sal == 0) &&
			    ...
			    ((cs_sah > 0) &&
			     (htset[1][cmpnfo[cmp].huffac] == 0))) {
				sprintf(errormessage,
					"huffman table missing in scan%i",
					scnc);
				delete huffr;
				errorlevel = 2;
				return false;
		...
		while (true) {
			...
			if (sta == -1) { // status -1 means error
				sprintf(errormessage,
					"decode error in scan%i / mcu%i", scnc,
					(cs_cmpc > 1) ? mcu : dpos);
				delete huffr;
				errorlevel = 2;
				return false;
	...
	if (!huffr->eof()) {
		sprintf(errormessage,
			"surplus data found after coded image data");
		errorlevel = 1;
	}
...
INTERN bool recode_jpeg(void)
{
	...
	while (true) {
		...
		while (true) {
			...
			if (cs_cmpc > 1) {
				if (jpegtype == 1) {
					...
					while (sta == 0) {
						...
						tmp = colldata[cmp][0][dpos] >>
						      cs_sal;
						block[0] = tmp - lastdc[cmp];
						lastdc[cmp] = tmp;

						// encode dc
						sta = jpg_encode_dc_prg_fs(
							huffw,
							&(hcodes[0]
								[cmpnfo[cmp]
									 .huffdc]),
			...
			if (sta == -1) { // status -1 means error
				sprintf(errormessage,
					"encode error in scan%i / mcu%i", scnc,
					(cs_cmpc > 1) ? mcu : dpos);
				delete huffw;
				errorlevel = 2;
				return false;
...
#if defined(USE_PLOCOI)
			(*coef) -= dc_coll_predictor(cmp,
						     dpos); // loco-i predictor
#else
	...
	for (cmp = 0; cmp < cmpc; cmp++) {
		...
		for (dpos = 1; dpos < cmpnfo[cmp].bc; dpos++) {
			...
			(*coef) += dc_coll_predictor(cmp,
						     dpos); // loco-i predictor
#else
			(*coef) += dc_1ddct_predictor(cmp,
						      dpos); // 1d dct predictor
#endif
		...
		for (bpos = 0; bpos < 64; bpos++) {
			...
			for (dpos = 0; dpos < cmpnfo[cmp].bc; dpos++)
				if ((colldata[cmp][bpos][dpos] > absmax) ||
					...
					sprintf(errormessage,
						"value out of range error: cmp%i, frq%i, val %i, max %i",
						cmp, bpos,
						colldata[cmp][bpos][dpos],
						absmax);
					errorlevel = 2;
...
// encode JPG header
	...
	for (cmp = 0; cmp < cmpc; cmp++) {
		...
		dev_size_cmp[cmp] = dev_size_zsr[cmp] + dev_size_zdh[cmp] +
				    dev_size_zdl[cmp] + dev_size_ach[cmp] +
				    dev_size_acl[cmp] + dev_size_dc[cmp];
#endif
	}
...
INTERN bool jpg_setup_imginfo(void)
{
	...
	for (cmp = 0; cmp < cmpc; cmp++) {
		if ((cmpnfo[cmp].sfv == 0) || (cmpnfo[cmp].sfh == 0) ||
		    (cmpnfo[cmp].qtable == nullptr) ||
	...
	mcuh = static_cast<int>(std::ceil(static_cast<float>(imgwidth) /
					  static_cast<float>(8 * sfvm)));
	mcuc = mcuv * mcuh;
	for (cmp = 0; cmp < cmpc; cmp++) {
		...
		cmpnfo[cmp].ncv = static_cast<int>(ceil(
			static_cast<float>(imgheight) *
			(static_cast<float>(cmpnfo[cmp].sfh) / (8.0 * sfhm))));
		cmpnfo[cmp].nch = static_cast<int>(ceil(
	...
	for (cmp = 0; cmp < cmpc; cmp++) {
		...
		for (bpos = 0; bpos < 64; bpos++) {
			colldata[cmp][bpos] = static_cast<short int *>(
				calloc(cmpnfo[cmp].bc, sizeof(short)));
			if (colldata[cmp][bpos] == nullptr) {
				sprintf(errormessage, MEM_ERRMSG);
				errorlevel = 2;
				return false;
		...
		zdstdata[cmp] = static_cast<unsigned char *>(
			calloc(cmpnfo[cmp].bc, sizeof(char)));
		eobxhigh[cmp] = static_cast<unsigned char *>(
			calloc(cmpnfo[cmp].bc, sizeof(char)));
		eobyhigh[cmp] = static_cast<unsigned char *>(
			calloc(cmpnfo[cmp].bc, sizeof(char)));
		zdstxlow[cmp] = static_cast<unsigned char *>(
			calloc(cmpnfo[cmp].bc, sizeof(char)));
		zdstylow[cmp] = static_cast<unsigned char *>(
			calloc(cmpnfo[cmp].bc, sizeof(char)));
		if ((zdstdata[cmp] == nullptr) || (eobxhigh[cmp] == nullptr) ||
		    (eobyhigh[cmp] == nullptr) || (zdstxlow[cmp] == nullptr) ||
		    (zdstylow[cmp] == nullptr)) {
			sprintf(errormessage, MEM_ERRMSG);
			errorlevel = 2;
			return false;
...
{
	unsigned int hpos =
		4; // current position in segment, start after segment header
	...
	case 0xC4: // DHT segment
		...
		while (hpos < len) {
			...
			jpg_build_huffcodes(&(segment[hpos + 0]),
					    &(segment[hpos + 16]),
					    &(hcodes[lval][rval]),
					    &(htrees[lval][rval]));
			htset[lval][rval] = 1;
	...
	case 0xDA: // SOS segment
		...
		if (cs_cmpc > cmpc) {
			sprintf(errormessage,
				"%i components in scan, only %i are allowed",
				cs_cmpc, cmpc);
			errorlevel = 2;
			return false;
		}
		...
		for (i = 0; i < cs_cmpc; i++) {
			...
			if ((cmpnfo[cmp].huffdc < 0) ||
			    (cmpnfo[cmp].huffdc >= 4) ||
			    (cmpnfo[cmp].huffac < 0) ||
			    (cmpnfo[cmp].huffac >= 4)) {
				sprintf(errormessage,
					"huffman table number mismatch");
				errorlevel = 2;
				return false;
			}
			hpos += 2;
	...
	case 0xC2: // SOF2 segment
		...
		if (lval != 8) {
			sprintf(errormessage,
				"%i bit data precision is not supported", lval);
			errorlevel = 2;
			return false;
		}
		...
		if ((imgwidth == 0) || (imgheight == 0)) {
			sprintf(errormessage,
				"resolution is %ix%i, possible malformed JPEG",
				imgwidth, imgheight);
			errorlevel = 2;
			return false;
		}
		if (cmpc > 4) {
			sprintf(errormessage,
				"image has %i components, max 4 are supported",
				cmpc);
			errorlevel = 2;
			return false;
		}
	...
	case 0xC3: // SOF3 segment
		...
		sprintf(errormessage,
			"sof3 marker found, image is coded lossless");
		errorlevel = 2;
		return false;
	...
	case 0xC5: // SOF5 segment
		...
		sprintf(errormessage,
			"sof5 marker found, image is coded diff. sequential");
		errorlevel = 2;
		return false;
	...
	case 0xC7: // SOF7 segment
		...
		sprintf(errormessage,
			"sof7 marker found, image is coded diff. lossless");
		errorlevel = 2;
		return false;
	...
	case 0xC9: // SOF9 segment
		...
		sprintf(errormessage,
			"sof9 marker found, image is coded arithm. sequential");
		errorlevel = 2;
		return false;
	...
	case 0xCA: // SOF10 segment
		// coding process: arithmetic extended sequential DCT
		sprintf(errormessage,
			"sof10 marker found, image is coded arithm. progressive");
		errorlevel = 2;
		return false;
	...
	case 0xCB: // SOF11 segment
		// coding process: arithmetic extended sequential DCT
		sprintf(errormessage,
			"sof11 marker found, image is coded arithm. lossless");
		errorlevel = 2;
		return false;
	...
	case 0xCD: // SOF13 segment
		...
		sprintf(errormessage,
			"sof13 marker found, image is coded arithm. diff. sequential");
		errorlevel = 2;
		return false;
	...
	case 0xCF: // SOF15 segment
		...
		sprintf(errormessage,
			"sof15 marker found, image is coded arithm. diff. lossless");
		errorlevel = 2;
		return false;
				...
				huffTree *actree, short *block)
{
	...
	for (bpos = 1; bpos < 64;) {
		...
		if (hc > 0) {
			...
			block[bpos++] =
				static_cast<short> DEVLI(s, n); // decode cvli
		} else if (hc == 0) { // EOB
			eob = bpos;
				...
				huffCodes *actbl, const short *block)
{
	...
	for (bpos = 1; bpos < 64; bpos++) {
		...
		if (block[bpos] != 0) {
			...
			while (z >= 16) {
				huffw->write_u16(actbl->cval[0xF0],
						 actbl->clen[0xF0]);
				z -= 16;
			}
			// vli encode
			   ...
			   int *rstw)
{
	...
	if (cmpnfo[(*cmp)].sfh > 1) { // to fix mcu order
		(*dpos) = ((*mcu) / mcuh) * cmpnfo[(*cmp)].sfh +
			  ((*sub) / cmpnfo[(*cmp)].sfv);
		(*dpos) *= cmpnfo[(*cmp)].bch;
		(*dpos) += ((*mcu) % mcuh) * cmpnfo[(*cmp)].sfv +
			   ((*sub) % cmpnfo[(*cmp)].sfv);
...
INTERN bool pjg_encode_dc(ArithmeticEncoder *enc, int cmp)
{
	...
	mod_len = INIT_MODEL_S(
		max_len + 1,
		(segm_cnt[cmp] > max_len) ? segm_cnt[cmp] : max_len + 1, 2);
	mod_res =
		INIT_MODEL_B((segm_cnt[cmp] < 16) ? 1 << 4 : segm_cnt[cmp], 2);
	...
	for (dpos = 0; dpos < bc; dpos++) {
		...
		ctx_avr = pjg_aavrg_context(c_absc, c_weight, dpos, p_y, p_x,
					    r_x); // AVERAGE context
		...
		if (coeffs[dpos] == 0) {
			...
			for (bp = clen - 2; bp >= 0; bp--) {
				shift_model(mod_res, snum,
					    bp); // shift in 2 contexts
				// encode/get bit
				bt = BITN(absv, bp);
...
INTERN bool pjg_encode_ac_high(ArithmeticEncoder *enc, int cmp)
{
	...
	// init models for bitlenghts and -patterns
	mod_len =
		INIT_MODEL_S(11, (segm_cnt[cmp] > 11) ? segm_cnt[cmp] : 11, 2);
	mod_res =
		INIT_MODEL_B((segm_cnt[cmp] < 16) ? 1 << 4 : segm_cnt[cmp], 2);
	...
	for (i = 1; i < 64; i++) {
		...
		for (dpos = 0; dpos < bc; dpos++) {
			...
			// calculate contexts (for bit length)
			ctx_avr =
				pjg_aavrg_context(c_absc, c_weight, dpos, p_y,
						  p_x, r_x); // AVERAGE context
			ctx_len = BITLEN1024P(ctx_avr); // BITLENGTH context
			// shift context / do context modelling (segmentation is done per context)
			shift_model(mod_len, ctx_len, snum);
...
INTERN bool pjg_encode_ac_low(ArithmeticEncoder *enc, int cmp)
{
	...
	mod_top = INIT_MODEL_B(
		(nois_trs[cmp] > 4) ? 1 << nois_trs[cmp] : 1 << 4, 3);
	...
	for (i = 2; i < 16; i++) {
		...
		if (b_x == 0) {
			for (; b_x < 8; b_x++) {
				coeffs_x[b_x] =
					colldata[cmp][zigzag[b_x + (8 * b_y)]];
				coeffs_a[b_x] =
					colldata[cmp][zigzag[b_x + (8 * b_y)]] -
					1;
				pred_cf[b_x] =
					icos_base_8x8[b_x * 8] *
					QUANT(cmp, zigzag[b_x + (8 * b_y)]);
			}
			b_x = 0;
			zdstls = zdstylow[cmp];
			edge_c = &p_x;
		...
		thrs_bp =
			(max_len > nois_trs[cmp]) ? max_len - nois_trs[cmp] : 0;
		...
		for (dpos = 0; dpos < bc; dpos++) {
			...
			if (coeffs[dpos] == 0) {
				...
				// encoding of residual
				bp = clen -
				     2; // first set bit must be 1, so we start at clen - 2
				...
				for (; bp >= thrs_bp; bp--) {
					shift_model(
						mod_top, ctx_abs >> thrs_bp,
						ctx_res,
						clen - thrs_bp); // shift in 3 contexts
					// encode/get bit
					bt = BITN(absv, bp);
...
INTERN inline int median_int(int *values, int size)
{
	...
	// return median
	return ((size % 2) == 0) ? (values[middle] + values[middle - 1]) / 2 :
				   values[middle];
}
...
{
	int len = strlen(base) +
		  ((extension == nullptr) ? 0 : strlen(extension) + 1) + 1;
...
INTERN inline void set_extension(char *filename, const char *extension)
{
	...
	extstr = (strrchr(filename, '.') == nullptr) ? strrchr(filename, '\0') :
						       strrchr(filename, '.');
...
{
	char *tmpname =
		static_cast<char *>(calloc(strlen(filename) + 1, sizeof(char)));
	char *extstr;
...
INTERN bool dump_coll(void)
{
	...
	for (cmp = 0; cmp < cmpc; cmp++) {
		...
		case 0: // standard collections
			for (bpos = 0; bpos < 64; bpos++)
				fwrite(colldata[cmp][bpos], sizeof(short),
				       cmpnfo[cmp].bc, fp);
			break;
		...
		case 2: // square collections
			...
			for (i = 0; i < 64;) {
				bpos = zigzag[i++];
				fwrite(&(colldata[cmp][bpos][dpos]),
				       sizeof(short), cmpnfo[cmp].bch, fp);
				if ((i % 8) == 0) {
					dpos += cmpnfo[cmp].bch;
					if (dpos >= cmpnfo[cmp].bc) {
						dpos = 0;
...
INTERN bool dump_zdst(void)
{
	...
	for (cmp = 0; cmp < cmpc; cmp++)
		if (!dump_file(basename, ext[cmp], zdstdata[cmp], 1,
			       cmpnfo[cmp].bc))
			return false;
...
INTERN bool dump_pgm(void)
{
	...
	for (cmp = 0; cmp < cmpc; cmp++) {
		...
		imgdata = (unsigned char *)calloc(cmpnfo[cmp].bc * 64,
						  sizeof(char));
		if (imgdata == NULL) {
			fclose(fp);
			sprintf(errormessage, MEM_ERRMSG);
		...
		for (dpos = 0; dpos < cmpnfo[cmp].bc; dpos++) {
			...
			dcpos = (((dpos / cmpnfo[cmp].bch) * cmpnfo[cmp].bch)
				 << 6) +
			...
			for (y = 0; y < 8; y++) {
				...
				for (x = 0; x < 8; x++) {
					xpos = ypos + x;
					pix_v = idct_2d_fst_8x8(cmp, dpos, x,
								y);
					pix_v = DCT_RESCALE(pix_v);
					pix_v = pix_v + 128;
					imgdata[xpos] = (unsigned char)CLAMPED(
						0, 255, pix_v);
				}
			}
		}
		...
		fprintf(fp, "# created by %s v%i.%i%s (%s) by %s\n", apptitle,
			appversion / 10, appversion % 10, subversion,
			versiondate, author);
		fprintf(fp, "%i %i\n", cmpnfo[cmp].bch * 8,
			cmpnfo[cmp].bcv * 8);
		fprintf(fp, "255\n");
