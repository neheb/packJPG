...
unsigned char *Reader::get_c_data()
{
	...
	auto c_data_copy = static_cast<unsigned char *>(
		std::malloc(data.size() * sizeof data[0]));
...
MemoryReader::MemoryReader(std::vector<std::uint8_t> bytes)
	: data_(std::move(bytes)), cbyte_(std::begin(data_))
{
}

MemoryReader::MemoryReader(const std::uint8_t *bytes, std::size_t size)
	: data_(bytes, bytes + size), cbyte_(std::begin(data_))
{
}
...
std::size_t MemoryReader::read(std::uint8_t *to, std::size_t num_to_read)
{
	...
	auto numRead =
		std::min(static_cast<std::size_t>(numAvailable), num_to_read);
...
std::size_t MemoryReader::read(std::vector<std::uint8_t> &into, std::size_t n,
			       std::size_t offset)
{
	const std::size_t num_available =
		get_size() -
		num_bytes_read(); // The number of bytes in the reader not yet read.
	const std::size_t num_to_read =
		std::min(n, num_available); // How many bytes will be read.
...
{
	auto num_to_skip = std::min(
		n, std::size_t(std::distance(cbyte_, std::end(data_))));
	cbyte_ += num_to_skip;
}

void MemoryReader::rewind_bytes(std::size_t n)
{
	auto num_to_rewind = std::min(
		n, std::size_t(std::distance(std::begin(data_), cbyte_)));
...
FileWriter::FileWriter(const std::string &file_path) : file_path_(file_path)
{
	...
	if (fptr_ != nullptr) {
		...
		std::setvbuf(fptr_, file_buffer_.data(), _IOFBF,
			     file_buffer_.capacity());
	} else {
		throw std::runtime_error("Unable to open " + file_path_ +
					 " for writing.");
	}
}
...
std::size_t FileReader::read(std::vector<std::uint8_t> &into,
			     std::size_t num_to_read, std::size_t offset)
{
...
StreamReader::StreamReader()
{
	...
	auto bytes_read = std::fread(buffer.data(), sizeof buffer[0],
				     buffer_capacity, stdin);
	while (bytes_read > 0) {
		stream_data.insert(std::end(stream_data), std::begin(buffer),
				   std::begin(buffer) + bytes_read);
		bytes_read = std::fread(buffer.data(), sizeof buffer[0],
					buffer_capacity, stdin);
	}
...
std::size_t StreamReader::read(std::vector<std::uint8_t> &into,
			       std::size_t num_to_read, std::size_t offset)
{
	return reader_->read(into, num_to_read, offset);
}
